...
fn main() {
    ...
    //create clones of partitions
    let clone1 = xs.clone();
    let clone2 = xs.clone();
    //create threads
    let t1 = thread::spawn(move || map_data(&clone1[0]));
    let t2 = thread::spawn(move || map_data(&clone2[1]));
    ...
    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
        //loop through each parition and create a thread to handle that partition.
    let mut join_handles : Vec<thread::JoinHandle<usize>> = Vec::new();

    for i in 0..num_partitions {
        let parted_clone = parted_data[i].clone();
        let parted_thread = thread::spawn(move || map_data(&parted_clone));

        join_handles.push(parted_thread);
    }
    ...
    let parted_sum = reduce_data(&intermediate_sums_2);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // will store each partition
    let mut total_partitions: Vec<Vec<usize>> = Vec::new();
    // will be used to get the correct number of elements for each partition.
    let mut modulus_value = v.len() % num_partitions;

    // Starting and ending values for each partition
    let mut start = 0;
    let mut end = size; 
    for i in 0..num_partitions {
        ...
        total_partitions.push(partition);
        // if mod_value is even then the partition will have equal amount of elements.
        if modulus_value == 0 {
            start += size;
            end += size;
        }
        //if mod_value is odd then partition will have one extra element.
        else {
            total_partitions[i].push(v[end]);
            start += size + 1;
            end += size + 1;
            modulus_value -= 1;
        }
